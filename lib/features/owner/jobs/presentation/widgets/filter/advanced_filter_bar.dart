...
class AdvancedFilterBar extends GetView<JobsController> {
  ...
  Future<void> _showAddFilterMenu(BuildContext context) async {
    ...
    final selectedField = await showMenu<String>(
      ...
      position: position,
      items: [
        PopupMenuItem(value: 'status', child: Text(AppStrings.jobs.status)),
        PopupMenuItem(value: 'date', child: Text(AppStrings.jobs.dateRange)),
      ],
    );
